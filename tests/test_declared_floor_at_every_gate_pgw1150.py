...
`registry.CompileContract` carries `numerics_floor`/`numerics_warn`, so the object
the FLEET mint parent hands the gate carries the family's declaration.

PROVENANCE is answered ONCE. If `numerics_probe.probe_compiled_graph` re-derives
...
`CompileContract` (fleet mint parent), and the local CLI's `CompileContract` (§4.28
...
from gen_worker.registry import CompileContract, extract_specs
...
def _registry_cell() -> CompileContract:
    """The FLEET mint parent's object: `pending.cfg` is the `spec.compile_contract()`
    the executor opened its `ArmingScope` with."""
    graph = extract_specs(endpoints.Fast)[0].compile_contract()
    assert graph is not None
    return graph


def _local_cell() -> CompileContract:
    """The §4.28 desktop object `cli.run` builds for `local_serve`.

    Spelled out field by field rather than through
    :meth:`CompileContract.from_declaration`, deliberately: this row has to stay
...
CFGS = {
    ...
    "fleet-mint-parent/registry.CompileContract": _registry_cell,
    "local-serve/cli.CompileContract": _local_cell,
}


def _undeclare(cfg: Any) -> Any:
    """The same cfg with the band removed — the negative control."""
    if isinstance(cfg, CompileContract):
...
def test_the_mint_parent_gate_judges_at_the_familys_declared_floor(
    ...
    assert numerics_ladder.PHASE_REFUSED in _phases(events), (
        "the compiled graph was DEGRADED, not DESTROYED — the gate scored it against "
...
def test_one_constructor_maps_a_declaration_onto_a_cell():
    """Both production sites go through `CompileContract.from_declaration`, so a
    must-survive field cannot reach one path and not the other — which is
    precisely how the band reached no gate at all."""
    decl = rig.declaration()
    for name, build in CFGS.items():
        graph = build()
        if not isinstance(graph, CompileContract):
            continue
        assert graph.numerics_floor == decl.numerics_floor, name
        assert graph.numerics_warn == decl.numerics_warn, name

    every: Dict[str, Any] = dataclasses.asdict(
        CompileContract.from_declaration(decl, lora_bucket=0))
