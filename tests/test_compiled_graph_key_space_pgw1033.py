"""pgw#1033 (re-cut by pgw#1059) — the STAMPED key is the compiled graph's identity;
...
* the **arm token** (``fleet_compiled_graphs.arm_identity``, ``arm1-<56 hex>``) — the
  pre-trace obligation identity. It exists before anything is compiled,
  which is exactly why it can name a pending mint — and it is NOT a compiled graph
  key (``compiled_graph_key.is_key`` is False on it);
* the **stamped** key (``aot_mint.compiled_graph_identity``, ``ck1-<56 hex>``, the
  four-axis graph x envelope x sm x toolchain digest) — what the exported
  compiled graph actually IS. It does not exist until the export finishes.
...
2. the pgw#672 quarantine gate read the computed ref while the two writers that
   ...
   record the STAMPED ref of the armed compiled graph — so the churn-loop gate was blind
   to the refs that actually fail;
3. ``aot_serve.note_aot_key`` had ONE caller, discovery — so a SELF-MINTED compiled graph
...
#: compile has run. NOT ck-shaped (pgw#1059): an obligation is not a compiled graph.
ARM_KEY = "arm1-" + "a" * 56
...
def test_the_memo_is_keyed_on_the_arm_key_the_next_arm_can_look_up(
    ...
    assert prior is minted, (
        ...
        "compiled_graph — every same-key re-arm pays a second full export")
...
def test_a_second_arm_of_the_same_identity_re_arms_instead_of_minting_again(
    ...
    assert again.armed, "the re-arm did not serve from the finalized compiled_graph"
...
def test_a_quarantined_STAMPED_ref_declines_the_next_arm_of_that_identity(
    ...
    assert again.eager_reason == EagerPhase.COMPILED_GRAPH_QUARANTINED, (
        "the arm re-minted an identity whose own compiled_graph was disproven seconds "
        "ago — a deterministic recipe rebuilds the same disproven compiled_graph")
    skipped = [
...
def test_an_ARM_TOKEN_is_never_classified_as_an_exported_ref(
    ...
    it read as one. The compiled graph's STAMPED key is a different space entirely.
    ...
    a pod). Registration is now a property of ``aot_serve.arm_compiled_graph``, so
    "an exported compiled graph's ref is recognized" is a fact about the WRAP and is
    ...
    assert not aot_serve.is_aot_ref(pending.ref), (
        "an OWED mint has no compiled_graph yet — nothing may claim its computed ref "
        "names an exported artifact")

    minted = _adopt(monkeypatch, pending)
    assert minted is not None
    assert minted.compiled_graph_key == STAMPED_KEY, (
        "the finalized compiled_graph carries the ARM token as its identity; the stamped "
...
def test_an_owed_mint_advertises_no_artifact_identity(
    ...
    assert sel.ref == minted.ref, (
        "an ADOPTED compiled_graph advertises the key stamped on the bytes it serves")
...
# `requested_compiled_graph_key` itself. This issue's fix was the INTERIM it announced:
