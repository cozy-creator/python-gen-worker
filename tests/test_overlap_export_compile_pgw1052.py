...
* compiled graphs REACH the pool while the export source is still open — proven
...
def test_compiled_graphs_reach_the_pool_as_they_export(tmp_path: Path) -> None:
    """The producer WAITS for the pool to finish an earlier compiled graph before it
    ...
    width = pool_mod.compiled_graph_workers(
        3, limit=2, vcpus=16, available_bytes=64 * _GIB,
        device_lock=True)
    assert width.workers == 2
    box = pool_mod.CompiledGraphCompilePool(
    ...
    def _reports_on_disk() -> int:
        ...
        return len(list((tmp_path / "pool").glob("compiled_graph-*/report.json")))

    def _source() -> Iterator[Tuple[str, Any]]:
        yield "unet/dim=0", _program(0)
        yield "unet/dim=1", _program(1)
        # The whole point: an earlier compiled graph COMPLETES (its child writes its
        # report) while this source is still open.
        await_progress(
            _reports_on_disk,
            lambda n: n >= 1,
            what="an compiled_graph child finishes while the source is open",
    ...
    assert overlap_seen and overlap_seen[0] >= 1, (
        "the pool never completed an compiled_graph while the export source was open "
        "— the phases are still sequential")
    assert box.ledger.compiled_graphs == 3
...
def _compiled_graph(name: str, h: int, w: int, module: nn.Module | None = None) -> Any:
...
def test_arrival_alias_merges_without_a_compile() -> None:
    ...
    first = _compiled_graph("unet/r0", *AREA_PRESERVING[0])
    assert canon.admit(first) is None
    for i, (h, w) in enumerate(AREA_PRESERVING[1:], start=1):
        keeper = canon.admit(_compiled_graph(f"unet/r{i}", h, w))
...
def test_arrival_collision_refuses_naming_the_axis() -> None:
    canon = aot_mint._ArrivalCanon()
    assert canon.admit(_compiled_graph("unet/r0", 4, 6)) is None
    with pytest.raises(aot_mint.MintRefused) as err:
        canon.admit(_compiled_graph("unet/r1", 6, 4, module=_OtherBlock()))
...
def test_arrival_and_late_alias_maps_merge_and_rehome() -> None:
    a, b, c = (_compiled_graph(f"unet/r{i}", 4, 6) for i in range(3))
...
def _mint(tmp: Path, *, compiled_graph_workers: int = 0) -> aot_mint.MintResult:
    pipe = types.SimpleNamespace(unet=TinyUNet())
    spec = aot_mint.ExportSpec(family=FAMILY, target="")
    return aot_mint.mint(pipe, spec, tmp, compiled_graph_workers=compiled_graph_workers)
...
def test_overlapped_and_serial_mints_share_one_compiled_graph_key(
    ...
    _declare()
    serial = _mint(tmp_path / "serial", compiled_graph_workers=1)
    ...
    assert overlapped.timings.get("compiled_graph_workers", 0) > 1, (
    ...
    # every compiled graph's metadata. Read it off one compiled graph rather than off a result
    # that no longer has a single metadata.
    pool_block = ((overlapped.compiled_graphs[0].metadata.get("mint_phases") or {})
                  .get("pool") or {})
