...
from gen_worker import aot_identity, compiled_graph_resolve
...
def _one(family: str = "sdxl", key: str = KEY, **kw: Any) -> Any:
    ...
    return compiled_graph_resolve.resolve_batch(family, [key], **kw)[0]
...
def test_the_body_carries_family_and_keys_and_nothing_else(stub) -> None:
    ...
    assert sent["path"] == compiled_graph_resolve.RESOLVE_PATH == "/v1/worker/compiled-graphs/resolve"
...
def test_the_single_key_wire_is_gone(stub) -> None:
    ...
    assert not hasattr(compiled_graph_resolve, "resolve")
...
def test_a_miss_is_an_answer_never_an_omission(stub) -> None:
    ...
    answer = _one()
    assert answer.graph is None and answer.miss
...
def test_a_per_key_fault_is_an_answer_and_keeps_its_own_token(
    ...
    "no compiled graph" would go pay for a full cold mint over a row the hub HAS."""
    _sent, resp = stub
...
def test_an_incomplete_answer_is_refused_before_the_cell_is_paid_for(
    ...
    """
    _sent, resp = stub
    # `cg_ref` is the dataclass field; its wire spelling is still `cell_ref`
    # (proto-lane vocabulary, pgw#1363).
    wire_key = {"cg_ref": "cell_ref"}.get(field, field)
    resp["r"] = _Resp(200, _batch(_hit_body(**{wire_key: ""})))
...
def test_a_non_key_is_refused_before_the_hub_is_dialled(stub) -> None:
    ...
    with pytest.raises(compiled_graph_resolve.CompiledGraphResolveRefused):
        compiled_graph_resolve.resolve_batch("sdxl", [KEY, "ck1-short"])
...
def test_a_missing_family_is_refused_before_the_hub_is_dialled(stub) -> None:
    ...
    with pytest.raises(compiled_graph_resolve.CompiledGraphResolveRefused):
        compiled_graph_resolve.resolve_batch("sdxl", [])
    assert not sent
...
def test_a_hit_builds_the_expected_identity_the_arm_gate_COMPARES(
    ...
    expected = graph.expected_identity()
...
def test_the_receipt_rides_the_answer_and_is_never_re_fetched(stub) -> None:
    ...
    resp["r"] = _Resp(200, _batch(_hit_body()))
    graph = _one().graph
    assert graph is not None and graph.receipt == "eyJ.aGVhZA.c2ln"

    import inspect

    source = inspect.getsource(compiled_graph_resolve)
    assert "compiled graphs/receipt" not in source
...
def test_the_transport_is_shaped_for_the_existing_delivery_path(stub) -> None:
    ...
    files = list(graph.transport.files)
    assert len(files) == 1
    entry = files[0]
...
def test_a_chunked_transport_carries_the_chunk_attributes(stub) -> None:
    ...
    graph = _one().graph
    assert graph is not None
    chunks = list(graph.transport.files[0].chunks)
    assert [c.len for c in chunks] == [2048, 2048]
    assert chunks[0].sha256 == "aa" * 32
    assert not hasattr(graph.transport.files[0], "chunk_size_bytes")
...
def test_materialize_delegates_and_adds_nothing(monkeypatch, stub) -> None:
    ...
    out = compiled_graph_resolve.materialize(graph, cache_dir=None)
    assert str(out).endswith("cell.tar.gz")
    assert seen["ref"] == graph.cg_ref
    assert seen["digest"] == graph.content_digest
    assert seen["presigned"] is graph.transport
...
def _attempt(monkeypatch, tmp_path, *, derive=None, resolve_batch=None,
    ...
    if resolve_batch is not None:
        monkeypatch.setattr(compiled_graph_resolve, "resolve_batch", resolve_batch)
...
def test_a_miss_degrades_as_miss_not_as_a_failure(monkeypatch, tmp_path) -> None:
    out = _attempt(
        ...
        resolve_batch=lambda _f, keys, **_k: tuple(
            compiled_graph_resolve.ResolveAnswer(compiled_graph_key=k, status="miss")
            for k in keys))
    assert not out.adopted
    ...
    out = _attempt(
        ...
        resolve_batch=lambda _f, keys, **_k: tuple(
            compiled_graph_resolve.ResolveAnswer(
                compiled_graph_key=k, status="hit", graph=_Graph())
            for k in keys),
        materialize=_boom)
    assert not out.adopted
...
def test_one_answer_per_key_in_request_order(stub) -> None:
    ...
    answers = compiled_graph_resolve.resolve_batch("sdxl", keys)
...
def test_a_short_answer_is_refused_not_read_as_misses(stub) -> None:
    ...
    resp["r"] = _Resp(200, _batch(_hit_body()))
    with pytest.raises(compiled_graph_resolve.CompiledGraphResolveRefused) as err:
        compiled_graph_resolve.resolve_batch("sdxl", [KEY, OTHER_KEY, THIRD_KEY])
...
def test_a_long_answer_is_refused_too(stub) -> None:
    ...
    with pytest.raises(compiled_graph_resolve.CompiledGraphResolveRefused) as err:
        compiled_graph_resolve.resolve_batch("sdxl", [KEY])
    assert err.value.code == "compiled_graph_resolve_short_answer"
...
def test_transposed_answers_are_refused(stub) -> None:
    ...
    with pytest.raises(compiled_graph_resolve.CompiledGraphResolveRefused) as err:
        compiled_graph_resolve.resolve_batch("sdxl", [KEY, OTHER_KEY])
    assert err.value.code == "compiled_graph_resolve_answer_out_of_order"
...
def test_distinctly_signed_answers_are_both_adopted(stub) -> None:
    ...
    answers = compiled_graph_resolve.resolve_batch("sdxl", [KEY, OTHER_KEY])
    assert [a.status for a in answers] == ["hit", "hit"]
    assert answers[0].graph is not None and answers[1].graph is not None
    assert answers[0].graph.receipt != answers[1].graph.receipt
...
def test_one_bad_answer_does_not_sink_its_siblings(stub) -> None:
    ...
    resp["r"] = _Resp(200, _batch(
        ...
        {"compiled_graph_key": OTHER_KEY, "status": "ambiguous", "found": False,
         "detail": "two rows"},
        _hit_body(compiled_graph_key=THIRD_KEY, receipt="eyJ.dGhpcmQ.c2ln")))
    answers = compiled_graph_resolve.resolve_batch("sdxl", [KEY, OTHER_KEY, THIRD_KEY])
...
def test_a_duplicate_key_is_refused_before_the_hub_is_dialled(stub) -> None:
    ...
    sent, _resp = stub
    with pytest.raises(compiled_graph_resolve.CompiledGraphResolveRefused) as err:
        compiled_graph_resolve.resolve_batch("sdxl", [KEY, OTHER_KEY, KEY])
...
def test_over_the_bound_is_refused_before_the_hub_is_dialled(stub) -> None:
    ...
    too_many = ["cg-key-v1-" + f"{i:056x}"
                for i in range(compiled_graph_resolve.MAX_RESOLVE_KEYS + 1)]
    with pytest.raises(compiled_graph_resolve.CompiledGraphResolveRefused) as err:
        compiled_graph_resolve.resolve_batch("sdxl", too_many)
    ...
    assert len(compiled_graph_resolve.resolve_batch("sdxl", too_many[:-1])) == \
        compiled_graph_resolve.MAX_RESOLVE_KEYS
