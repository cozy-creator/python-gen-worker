...
def _compiled_graph(tmp_path: Path, meta_bytes: bytes, name: str = "compiled_graph.tar.gz") -> Path:
...
def test_a_legitimate_envelope_still_reads(tmp_path: Path):
    ...
    assert artifact_meta.read_metadata(_compiled_graph(tmp_path, raw)) == meta
...
def test_a_decompression_bomb_in_the_envelope_is_refused_before_it_is_read(
    ...
    bomb = b"\0" * (128 << 20)
    artifact = _compiled_graph(tmp_path, bomb)
...
def test_the_bound_is_exact_at_both_sides(tmp_path: Path):
    ...
    assert len(at) == limit
    assert artifact_meta.read_metadata(_compiled_graph(tmp_path, at, "at.tar.gz"))["pad"]

    over = (pad_key + "x" * (limit - len(pad_key) - 1) + '"}').encode()
    assert len(over) == limit + 1
    with pytest.raises(artifact_meta.ArtifactMetadataError):
        artifact_meta.read_metadata(_compiled_graph(tmp_path, over, "over.tar.gz"))
