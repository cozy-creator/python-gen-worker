...
def test_compiler_policy_is_not_a_worker_wire_surface() -> None:
    """TCG owns compiler policy; the worker chooses only process posture."""
    import inspect

    from gen_worker import aot_mint

    assert "inductor_configs" not in inspect.signature(pool.EntryJob).parameters
    assert "inductor_configs" not in inspect.signature(
        pool.EntryCompilePool,
    ).parameters
    assert not hasattr(aot_mint, "_entry_configs")
