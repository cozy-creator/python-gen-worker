...
def test_sibling_functions_share_one_compile_contract() -> None:
    ...
    a = specs["generate"].compile_compiled_graph()
    b = specs["generate_turbo"].compile_compiled_graph()
...
def test_union_is_class_scoped_not_cross_class() -> None:
    ...
    assert specs["generate_turbo"].compile_compiled_graph().guidance_scales == ()
...
def test_per_function_text_pins_digest_as_one_dual_pin_contract() -> None:
    ...
    a = specs["generate"].compile_compiled_graph()
    b = specs["edit_image"].compile_compiled_graph()
...
def test_model_ref_label_mirrors_registry_wire_forms() -> None:
    ...
    assert Hub("acme/wai-illustrious", tag="latest").label \
