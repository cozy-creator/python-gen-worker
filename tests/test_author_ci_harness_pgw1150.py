...
def test_a_healthy_compiled_graph_records_the_gates_own_cosine(
        tmp_path, monkeypatch, declared, on_the_line):
    armed_compiled_graph(tmp_path, monkeypatch, declared, 1.0, verify_numerics=False)
    record = record_file(tmp_path)
...
def test_the_declared_numerics_floor_reaches_the_gate_at_all(declared):
    ...
    .CompileCompiledGraph` — which carried no `numerics_floor` field at all. So every
    ...
    assert compiled_graph.numerics_floor == endpoints.FLOOR
    assert compiled_graph.numerics_warn == endpoints.WARN
    bar = numerics_ladder.declared_thresholds(compiled_graph)
    ...
    assert "numerics_floor" not in compiled_graph.contract_facts()
    assert "numerics_warn" not in compiled_graph.contract_facts()
