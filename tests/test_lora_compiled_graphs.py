"""Lora-bucket compile compiled graphs (gw#561, SDK v2 pgw#647): the decorator-level
...
def _cfg(
    ...
    """The enriched compile-compiled graph configuration the machinery consumes in v2
    (``EndpointSpec.compile_compiled_graph()``): lora_bucket lives here, never on
    ``Compile``."""
...
def w8a8_tree(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    root = tmp_path_factory.mktemp("loracompiledgraphs") / "src"
    unet = UNet2DModel(
...
def test_enable_compiled_rolls_back_branches_when_eager(plain_pipe: Any) -> None:
    ...
    cfg = _cfg("loracompiledgraphs-test", lora_bucket=32)
...
def test_enable_compiled_w8a8_fail_closed_keeps_contract(w8a8_pipe: Any) -> None:
    cfg = _cfg("loracompiledgraphs-test", lora_bucket=128)
...
def test_rank_buckets_cover_declared_compiled_graphs() -> None:
...
def test_discovery_carries_lora_bucket() -> None:
    ...
    # The compile machinery consumes the enriched CompileCompiledGraph, which folds
    ...
    assert compiled_graph.family == "f"
    assert compiled_graph.contract_facts()["lora_bucket"] == 64
...
def test_enable_compiled_skips_execution_lane_on_component_slot_without_target() -> None:
    ...
    vae = _Vae()
    cfg = _cfg("loracompiledgraphs-test", lora_bucket=64)
    armed = provision.enable_compiled(vae, cfg, cache_dir=None, artifact=None).armed
    assert armed is False


def test_delivered_lora_compiled_graph_on_component_slot_is_ordinary_miss(
    ...
    # pgw#1181: the delivered compiled graph this used to build is a
