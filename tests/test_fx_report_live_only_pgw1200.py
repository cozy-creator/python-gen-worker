...
class _FakeFxEntry:
    """What torch pickles into an fxgraph cache entry, at the two attributes
    the report reads."""
...
def _write_live_entry(root: Path, key: str, name: str = "entry.bin") -> Path:
    d = root / "fxgraph" / key[1:3] / key
    d.mkdir(parents=True, exist_ok=True)
    p = d / name
    p.write_bytes(pickle.dumps(_FakeFxEntry(guards="L['x'].size()[0] == 2")))
    return p
...
def test_the_report_still_runs_and_names_the_live_cache(
    ...
    live = tmp_path / "live"
    _write_live_entry(live, "fkey00000001")
    _write_live_entry(live, "fkey00000002")
...
def test_a_missing_cache_dir_is_named_not_silently_zero(
    ...
    """Distinguishing "no cache directory" from "a directory with no entries"
...
def test_the_live_census_reads_real_torch_entry_files(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Dotfiles and empty key dirs are not entries — torch writes lock and
    temp siblings, and counting them would inflate the one number left."""
    live = tmp_path / "live"
    entry = _write_live_entry(live, "fkey00000001")
    (entry.parent / ".lock").write_bytes(b"")
...
def test_an_unreadable_entry_never_breaks_the_report(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    live = tmp_path / "live"
    d = live / "fxgraph" / "ke" / "fkey00000001"
    d.mkdir(parents=True)
    (d / "entry.bin").write_bytes(b"not-a-pickle")
    monkeypatch.setenv("TORCHINDUCTOR_CACHE_DIR", str(live))
