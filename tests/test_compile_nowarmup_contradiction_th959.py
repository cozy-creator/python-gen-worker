...
Compile compiled graphs are minted and proven by warmup execution (self-mint boots a
...
def test_custom_warmup_method_resolves_the_contradiction() -> None:
    @endpoint(
        ...
        resources=Resources(gpu=True),
        compile=_compile(),
        warmup=NoWarmup("derived plan opted out; custom warmup proves compiled_graphs"),
    )
    class CustomWarmup:
        def setup(self, pipeline: str) -> None:
            self.pipeline = pipeline
