...
def test_run_envelope_workspace_scope() -> None:
    ...
    assert req_only.run_id is None
    assert req_only.workspace_scope_id == "req-a"
    assert _workspace_scope_id("req-a", None) == "req-a"

    with_run = RequestContext("req-b", run_id=" run-123 ")
    assert with_run.run_id == "run-123"
    assert with_run.workspace_scope_id == "run-123"
    assert with_run.partition_context()["run_id"] == "run-123"
    assert _workspace_scope_id("req-b", "run-123") == "run-123"

    blank_run = RequestContext("req-c", run_id="   ")
    assert blank_run.run_id is None
    assert blank_run.workspace_scope_id == "req-c"
    assert blank_run.partition_context()["run_id"] is None
