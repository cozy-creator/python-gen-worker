...
# --------------------------------------------------------------------------
# 8. the acceptance grep, as a permanent test
# --------------------------------------------------------------------------

_FAMILY_TOKENS = re.compile(
    r"\b(sdxl|sd15|sd15_sd2|sd21|flux|flux1|flux2|wan|wan21|wan22|ltx|ltx2|"
    r"zimage|z_image|qwen|auraflow|hidream|pony|illustrious|stablediffusion)\b",
    re.IGNORECASE,
)
_ENGINE_FILES = (
    "convert/repackage.py",
    "convert/layout.py",
    "convert/layout_spec.py",
    "convert/repack_spec.py",
    "convert/registry.py",
    "convert/base_model_families.py",
    "component_vocab.py",
)
...
def _code_only(path: Path) -> list[tuple[int, str]]:
    ...
    Family names surviving in *comments* are provenance and are allowed; a
    family name in live code is the thing this migration removed.
    """
    ...
    with open(path, "rb") as handle:
        ...
        for tok in tokenize.tokenize(handle.readline):
            ...
            if tok.type == tokenize.STRING and tok.string.lstrip("rbuf").startswith(('"""', "'''")):
                continue  # docstring
            if tok.start[0] != prev_end[0] or tok.type == tokenize.NAME:
                out.append((tok.start[0], tok.string))
            prev_end = tok.end
    return out


def test_the_engine_names_no_family_in_live_code() -> None:
    root = Path(__file__).resolve().parents[1] / "src" / "gen_worker"
    offenders: list[str] = []
    for rel in _ENGINE_FILES:
        for lineno, text in _code_only(root / rel):
            if _FAMILY_TOKENS.search(text):
                offenders.append(f"{rel}:{lineno}: {text}")
    assert offenders == [], (
        "family knowledge leaked back into the generic engine:\n" + "\n".join(offenders))
