...
def test_qweight_and_wscale_layouts_round_trip(
    ...
    """The fragment interleave and the lane swizzle are both bijective — the decoder replays the packer backwards rather than re-deriving index math."""
...
def test_qweight_row_is_not_the_output_channel() -> None:
    """Guards the finding that makes a naive 'just transpose it' converter silently wrong: the packed row index is NOT the logical output channel."""
...
def test_split_fused_qkv_is_exact() -> None:
    """nunchaku fuses q/k/v; diffusers keeps them separate."""
...
def test_fold_to_dense_matches_the_two_branch_reference() -> None:
    """The any-hardware fallback must be the WHOLE linear: the smoothing divide and the low-rank branch folded in, not just the 4-bit weight."""
...
def test_swap_splits_a_fused_qkv_into_three_dense_linears() -> None:
    """The full native path in the any-hardware mode: one fused nunchaku ``to_qkv`` becomes three working diffusers projections."""
    dim = 3072
    model = _Block(dim)
...
def test_native_sm_window_is_blackwell_only() -> None:
    """torch's own nvfp4 gate admits sm_89/sm_90, but neither has fp4 tensor cores."""
...
def test_int4_is_a_typed_refusal_on_the_detected_artifact() -> None:
    """int4 has no native engine and no other engine is installed, so it is refused BY NAME on the artifact — not by failing to find a loader."""
...
def test_int4_is_refused_before_any_native_probe(monkeypatch) -> None:
    """The int4 arm must precede the silicon check, so an int4 artifact on a Blackwell card still gets the int4 message rather than a hardware one."""
...
def test_fp4_carries_the_native_engine_reason_when_it_cannot(monkeypatch) -> None:
    """One engine, one reason — and it is the NATIVE one, named verbatim, so an operator reads about silicon rather than about a missing wheel."""
...
def test_svdq_linear_forward_matches_the_folded_reference() -> None:
    """SvdqLinear's two-branch fp4 forward vs the dense fold of the same checkpoint — the fp4 path may only differ by activation-quant error."""
