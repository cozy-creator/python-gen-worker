...
def _pending(tmp_path: Path, publisher: Any) -> fleet_compiled_graphs.PendingSelfMint:
    root = tmp_path / "mint"
    return fleet_compiled_graphs.PendingSelfMint(
        family="micro-diffusion", arm_token=ARM_A, ref="r#x", cfg=_Cfg(),
        target=root / "compiled_graph.tar.gz", mint_root=root, publisher=publisher,
...
def _arming(monkeypatch: pytest.MonkeyPatch, *, ok: bool,
    ...
    def _arm(pipe: Any, cfg: Any, cache_dir: Any, artifact: Path,
        ...
        return AdoptOutcome.miss("compile_compiled_graph_failed", "the card said no")
...
def test_a_trusted_pod_writes_the_compiled_graph_to_local_cas(
    ...
    kept = local_compiled_graph_store.lookup(KEY_A)
...
def test_the_cas_copy_exists_BEFORE_the_arm_runs(
    ...
    assert local_compiled_graph_store.lookup(KEY_A) is not None, (
        "the gate passed and the compiled_graph was never promoted to admitted")
...
def test_an_arm_refusal_QUARANTINES_the_bytes_instead_of_deleting_them(
    ...
    assert local_compiled_graph_store.lookup(KEY_A) is None, (
        "a compiled_graph that failed its arm must never be armable from the store")
...
def test_a_pending_publish_is_retried_on_the_NEXT_boot(
    ...
    for t in fleet_compiled_graphs.resume_owed_publishes(sink):  # type: ignore[arg-type]
        t.join(timeout=30)

    assert [p.name for p in sink.published] == ["compiled_graph.tar.gz"]
    kept = local_compiled_graph_store.lookup(KEY_A)
    assert kept is not None
...
def test_a_published_compiled_graph_is_not_re_uploaded_next_boot(
    store: Path, tmp_path: Path, quiet: List[Tuple[str, str]],
) -> None:
    local_compiled_graph_store.store(_artifact(tmp_path), key=KEY_A, family="f",
                           arm_token=ARM_A, sink=local_compiled_graph_store.SINK_DELIVERED)
    sink = _Sink()
    assert list(fleet_compiled_graphs.resume_owed_publishes(sink)) == []  # type: ignore[arg-type]
    assert sink.published == []


def test_a_compiled_graph_with_no_sink_by_design_owes_no_publish(
    store: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
    quiet: List[Tuple[str, str]],
) -> None:
    ...
    assert list(fleet_compiled_graphs.resume_owed_publishes(None)) == []
...
def test_no_terminus_deletes_a_compiled_graph_the_store_does_not_hold(
    store: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
    quiet: List[Tuple[str, str]],
) -> None:
