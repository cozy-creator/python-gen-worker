...
def test_hf_downloader_errors_when_hub_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "gen-worker" in str(e.value)
...
def test_hf_downloader_builds_allow_patterns_and_skips_optional_components(
    ...
    assert "unet/*fp16*.safetensors" in allow
    assert "vae/*fp16*.safetensors" in allow
    assert "text_encoder/*fp16*.safetensors" in allow
    assert "tokenizer/*" in allow
    assert "scheduler/*" in allow
