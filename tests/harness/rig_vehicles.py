...
class Vehicle:
    ...
    #: () -> the ``registry.CompileCompiledGraph`` the delegation boundary carries.
    compile_compiled_graph: Callable[[], Any]
...
def _micro_compiled_graph(bucket: int = 0) -> Any:
    from gen_worker.registry import CompileCompiledGraph
...
for p in %(paths)r:
    sys.path.insert(0, p)
# A MISS is not a crash and its reason lives in the typed `aot-compiled graphs` events.
...
from types import SimpleNamespace as _CompiledGraphNS
...
try:
    ...
    compiled graph = _CompiledGraphNS(
...
if compiled graph is not None:
    meta = aot_serve.unpack_metadata(Path(compiled graph.artifact))
    out.update({
    ...
    outcome = provision.arm_aot(
        pipe, cfg, Path(%(cache)r), Path(compiled graph.artifact),
...
def _micro_adopt_source_for(bucket: int) -> Any:
    ...
    SAME `CompileCompiledGraph` the mint was keyed on, and a hardcoded `lora_bucket=0`
...
def _micro_parent_for(bucket: int, pipeline_cls: str = "MicroPipeline",
    ...
    def _build(tree: Path, device: str) -> Tuple[Any, Any]:
        ...
        cfg = compiled_graph() if compiled_graph is not None else _micro_compiled_graph(bucket)
        return pipe, cfg
...
MICRO_LORA = Vehicle(
    ...
    build_checkpoint=_micro_checkpoint,
    checkpoint_bytes=_micro_checkpoint_bytes,
    compile_compiled_graph=lambda: _micro_compiled_graph(MICRO_LORA_BUCKET),
...
MICRO_LORA_PLAIN_PARENT = Vehicle(
    ...
    adopt_source=_micro_adopt_source_for(0),
    parent_pipe=_micro_parent_for(MICRO_LORA_BUCKET),
    covers=("the pgw#999 design question: a bucket-bearing compiled_graph offered to a "
            "parent on the plain lane"),
    expect="red",
    expect_note=("the arm gate refuses the exact named compiled_graph on lane drift — "
                 "the adopting pod must carry the compiled_graph's own lora bucket "
...
def _micro_4d_compiled_graph() -> Any:
    from gen_worker.registry import CompileCompiledGraph
...
MICRO_4D = Vehicle(
    ...
    build_checkpoint=_micro_checkpoint,
    checkpoint_bytes=_micro_checkpoint_bytes,
    compile_compiled_graph=_micro_4d_compiled_graph,
    adopt_source=_micro_4d_adopt_source,
    parent_pipe=_micro_parent_for(0, "MicroGridPipeline", _micro_4d_compiled_graph),
...
MICRO_LORA16 = Vehicle(
    ...
    build_checkpoint=_micro_checkpoint,
    checkpoint_bytes=_micro_checkpoint_bytes,
    compile_compiled_graph=lambda: _micro_compiled_graph(MICRO_LORA16_BUCKET),
    ...
    covers=(f"pgw#1073: everything `micro-lora` covers at a SECOND rank "
            ...
            f"key axis, so this compiled_graph's lane is `lora16`, disjoint from "
...
def _micro_conv_compiled_graph() -> Any:
    from gen_worker.registry import CompileCompiledGraph
...
MICRO_CONV = Vehicle(
    ...
    build_checkpoint=_micro_checkpoint,
    checkpoint_bytes=_micro_checkpoint_bytes,
    compile_compiled_graph=_micro_conv_compiled_graph,
...
def _w8a8_vehicle(name: str, bucket: int) -> Vehicle:
    ...
    return Vehicle(
        ...
        checkpoint_bytes=_micro_checkpoint_bytes,
        compile_compiled_graph=lambda: _micro_compiled_graph(bucket),
...
def _micro_escape_compiled_graph() -> Any:
    from gen_worker.registry import CompileCompiledGraph
...
MICRO_ESCAPE = Vehicle(
    ...
    build_checkpoint=_micro_checkpoint,
    checkpoint_bytes=_micro_checkpoint_bytes,
    compile_compiled_graph=_micro_escape_compiled_graph,
    adopt_source=_micro_escape_adopt_source,
    parent_pipe=_micro_parent_for(0, "MicroEscapePipeline", _micro_escape_compiled_graph),
    gpu_only=True,
...
def _declaration_module(module: str, family: str) -> Any:
    ...
    silent failure: ``compile_compiled_graph()`` returns a compiled graph whose family nothing has
...
def _micro_pad32_compiled_graph(family: str = "micro-pad32") -> Any:
...
MICRO_PAD32 = Vehicle(
    ...
    build_checkpoint=_micro_checkpoint,
    checkpoint_bytes=_micro_checkpoint_bytes,
    compile_compiled_graph=_micro_pad32_compiled_graph,
    adopt_source=_micro_pad32_adopt_source_for("micro-pad32"),
    parent_pipe=_micro_parent_for(0, "MicroPad32Pipeline", _micro_pad32_compiled_graph),
...
MICRO_PAD32_BRANCHY = Vehicle(
    ...
    build_checkpoint=_micro_checkpoint,
    checkpoint_bytes=_micro_checkpoint_bytes,
    compile_compiled_graph=lambda: _micro_pad32_compiled_graph("micro-pad32-branchy"),
    adopt_source=_micro_pad32_adopt_source_for("micro-pad32-branchy"),
    parent_pipe=_micro_parent_for(
        0, "MicroPad32BranchyPipeline",
        lambda: _micro_pad32_compiled_graph("micro-pad32-branchy")),
    expect="green",
    expect_note="",
...
def _micro_rope_compiled_graph() -> Any:
    from gen_worker.registry import CompileCompiledGraph
...
MICRO_ROPE = Vehicle(
    ...
    build_checkpoint=_micro_checkpoint,
    checkpoint_bytes=_micro_checkpoint_bytes,
    compile_compiled_graph=_micro_rope_compiled_graph,
