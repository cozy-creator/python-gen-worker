"""pgw#904: the rig's second-process adoption fetches the EXACT cell the
publish leg produced — by checkpoint id, never by listing. Catalog discovery
(`aot_cells`) is deleted; a rig that re-listed the family repo and ranked
...
def fetch_named_cell(
    ...
    if want and got != want:
        raise RuntimeError(
            f"named cell {checkpoint_id} bytes refused: {got} != {want}")
    dest = dest_dir / f"{checkpoint_id.split(':', 1)[-1]}.tar.gz"
    dest.write_bytes(raw)
    return dest


__all__ = ["fetch_named_cell"]
