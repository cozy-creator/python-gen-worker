...
def test_advisory_intent_errors_decline_without_latching() -> None:
    """Symptom 3, worker half: an unknown-function FUNCTION_READY declared advisory is declined typed while the rest of the command is accepted and the process keeps serving."""
...
def test_command_seq_conflict_without_new_mandatory_work_fails_open() -> None:
    """Symptom 1, worker half: a goal_id drift at a fixed command_seq whose intents are all already registered rejects the resend but must NOT latch the process."""
    registry = IntentRegistry("release-1", ["generate"])
    first = _base_command(
        registry,
...
def test_mandatory_intent_error_still_fails_closed() -> None:
    """A genuinely mandatory intent (request-blocking MATERIALIZE) with a validation error must still latch the process."""
    registry = IntentRegistry("release-1", ["generate"])
