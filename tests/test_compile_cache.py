...
def test_w8a8_enable_refusal_carries_exact_reason(tmp_path, monkeypatch):
    ...
    assert "no compiled graph artifact delivered" in message
...
def test_guidance_regimes_are_an_envelope_axis():
    """SDK v2: warm guidance regimes live on the enriched CompileContract (from
...
def test_endpoint_compile_reaches_spec():
    ...
    graph = specs[0].compile_contract()
    assert graph is not None
    assert graph.shapes == ((768, 768),)
    assert graph.text_len == 0
    ...
    assert graph.guidance_scales == (0.0, 5.0)  # class union, sorted
...
# ---------------------------------------------------------------------------
# resident prep-mode drift (off <-> vae_only) converges to the compiled graph
# ---------------------------------------------------------------------------
