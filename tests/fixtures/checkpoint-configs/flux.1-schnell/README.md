# FLUX.1-schnell checkpoint configs — the SERVED packaging, fetched verbatim

Config-only. **No weights**, and no multi-MB tokenizer vocab blobs. Pulled
whole-file from the hub's own resolve manifest for `tensorhub/flux1-schnell`,
...
**`scheduler/scheduler_config.json`: `shift` = 1.0, `use_dynamic_shifting` = False.**
...
**`transformer/config.json`: `guidance_embeds` = False.**
