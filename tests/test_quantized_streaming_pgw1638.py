...
BACKPORT NOTE (pin window `bf6c3bbb`): the master version of this file also
carries `test_the_lane_cast_would_have_rounded_the_scale_grid`, the red arm
for the second half of the fix — that the pgw#1623 lane cast must not round an
F32 block-scale grid to the lane's bf16. THIS WINDOW PREDATES pgw#1623: there
is no `_cast_to_lane` and no `_assert_lane_dtype` in this engine, so there is
nothing here to exempt and nothing to neuter. The arm is omitted rather than
faked, and it returns with the window.
"""
...
def test_a_tree_whose_rule_this_image_cannot_decode_is_refused(
    ...
    engine = _engine(article["tree"])
    with pytest.raises(decode_set_mod.ContractNotDecodableError) as caught:
        engine.build(
            article["pipeline_cls"], checkpoint_dir=article["tree"], lane=Lane())
    assert CONTRACT_HF_FP8_BLOCKWISE in str(caught.value)
