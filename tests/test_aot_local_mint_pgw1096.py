...
* nothing in the tree could address a locally-stored AOT compiled graph (``local_compiled_graphs``
...
child's own compiled graph passes — structurally, so it cannot drift into a weaker one.
"""
...
from gen_worker import aot_resume, compiled_graph_key, fleet_compiled_graphs, local_compiled_graph_store
...
ARM_B = fleet_compiled_graphs.ARM_SCHEME + "-" + "2" * 56
...
def _armable_artifact(tmp_path: Path, *, key: str = KEY_A) -> Path:
    """A compiled graph with a READABLE envelope — for the tests that reach the ARM.

    pgw#1098: `_arm_exported_compiled_graph` refuses `compiled_graph_envelope_unreadable` before
...
def test_a_compiled_graph_is_addressed_by_the_same_ck1_key_the_hub_store_uses(
    store: Path, tmp_path: Path,
) -> None:
    """§4.28's "one identity, two stores": the local address IS the stamped key.

    RED before: no local address existed for an AOT compiled graph at all — `local_compiled_graphs`
    keys on a flavor LABEL (sku/torch/lane), a space with no `ck1` in it.
    """
    compiled_graph = local_compiled_graph_store.store(
...
# ---------------------------------------------------------------------------
# 2. RED — a corrupted compiled graph is refused, and costs exactly one re-mint
# ---------------------------------------------------------------------------


def test_corrupting_a_stored_compiled_graph_refuses_admission_and_drops_it(
    store: Path, tmp_path: Path,
) -> None:
    ...
    assert not local_compiled_graph_store.compiled_graph_dir(KEY_A).exists(), (
        "a refused compiled_graph must be DROPPED — leaving it costs a re-verify (and a "
    ...
    assert local_compiled_graph_store.lookup_for_arm(ARM_A) is None, (
        "the memo must not resurrect a compiled_graph the digest refused")
...
def test_the_memo_turns_a_pre_trace_identity_into_the_ck1_key(
    ...
    hit = local_compiled_graph_store.lookup_for_arm(ARM_A)
    assert hit is not None and hit.key == KEY_A
    assert local_compiled_graph_store.lookup_for_arm(ARM_B) is None


def test_a_moved_toolchain_is_an_HONEST_re_mint_and_leaves_the_old_compiled_graph_alone(
    store: Path, tmp_path: Path,
) -> None:
...
def test_the_worker_learns_it_is_untrusted_only_from_the_hubs_typed_refusal(
    ...
    for unrelated in ("compiled_graph_publish_forged_axis", "compiled_graph_publish_quota_exceeded",
                      "compiled_graph_publish_family_undeclared", "http_429", ""):
        assert local_compiled_graph_store.note_refusal(unrelated, "detail") is False
        assert local_compiled_graph_store.trust_class() == "", (
            f"{unrelated!r} says something about the COMPILED_GRAPH or the moment, never "
...
def test_no_env_declares_the_trust_class(store: Path) -> None:
    ...
    assert local_compiled_graph_store.ENV_STORE_DIR == "GEN_WORKER_LOCAL_COMPILED_GRAPHS_DIR", (
...
def test_the_local_store_has_no_publish_path(store: Path) -> None:
    """A compile compiled graph is user-generated EXECUTABLE code. Accepting one from a
    ...
    assert not hits, f"the local compiled_graph store must never grow a publish path: {hits}"
    ...
    for word in ("upload", "publish", "presign", "put_object", "post", "http"):
        ...
        assert not bad, f"the local compiled_graph store references {bad}"
...
def test_the_production_resume_bank_still_roots_in_the_local_store(
    ...
    its bank was rooted through `local_compiled_graphs.store_root()` — the module
    ...
    assert bank.is_relative_to(local_compiled_graph_store.store_root())
    ...
    assert "local_compiled_graphs" not in names, (
...
def armable(monkeypatch: pytest.MonkeyPatch) -> List[Path]:
    ...
    monkeypatch.setattr(fleet_compiled_graphs.aot_serve, "note_aot_key", lambda k: None)
...
def test_a_local_compiled_graph_that_cannot_arm_is_dropped_not_retried_forever(
...
def test_a_local_compiled_graph_that_does_not_describe_this_runtime_is_refused_by_FACT(
    store: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        fleet_compiled_graphs.provision, "arm_aot",
        lambda *a, **k: pytest.fail("a diverging compiled_graph must never reach the arm"))
    monkeypatch.setattr(
...
def test_the_local_compiled_graph_and_the_childs_compiled_graph_pass_the_SAME_gate(
    store: Path,
) -> None:
...
def test_an_untrusted_refusal_keeps_the_compiled_graph_instead_of_discarding_it(
    store: Path, tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    local_compiled_graph_store.store(
        ...
        sink=local_compiled_graph_store.SINK_OWED)
    ...
    fleet_compiled_graphs._publish_async(
        ...
        local_compiled_graph_store.lookup(KEY_A).artifact,  # type: ignore[union-attr]
...
def test_cozy_local_keeps_its_compiled_graph_without_ever_asking_a_hub(
...
def test_a_probe_pod_keeps_its_compiled_graph_because_its_publish_is_DISARMED(
    store: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    """pgw#980 removes `compiled graphs.publish_intent` from the parent's allowlist, so a
...
def test_nothing_evicts_and_the_accumulation_is_enumerable(
    ...
    for key in (KEY_A, KEY_B):
        local_compiled_graph_store.store(_artifact(tmp_path), key=key, family="f")
