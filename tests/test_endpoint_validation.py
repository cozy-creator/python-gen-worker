"""Tests for endpoint directory validation helpers."""
...
class TestEndpointValidation(unittest.TestCase):
    ...
    def test_requires_pyproject_name(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / "Dockerfile").write_text("FROM scratch\n", encoding="utf-8")
            (root / "cozy.toml").write_text(
    ...
    def test_accepts_non_slug_endpoint_name(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / "Dockerfile").write_text("FROM scratch\n", encoding="utf-8")
            (root / "cozy.toml").write_text(
