...
# ---------------------------------------------------------------------------
# 1. the axis sets, and the standing sweep
# ---------------------------------------------------------------------------


def test_no_adoption_lane_declares_sku_as_an_identity_axis() -> None:
    """Both cell lanes name their refusal axes in one place, and ``sku`` is
    in neither. ``sm`` is in both: it is what the compiled code IS."""
    for axes in (aot_serve.IDENTITY_AXES, compile_cache.IDENTITY_AXES):
        assert "sku" not in axes
        assert "sm" in axes


#: (file, enclosing function) -> why a ``"sku"`` comparison there is NOT an
#: adoption gate. Anything else is a reintroduction of the pgw#765 defect.
_ALLOWED_SKU_COMPARISONS = {
    ("aot_cells.py", "_candidates"):
        "the ratified SELECTION PREFERENCE: same-SKU sorts first for "
        "autotune affinity, and every row stays in the list either way "
        "(asserted by test_cross_sku_cell_is_still_a_candidate).",
    ("trt_engine.py", "verify"):
        "a TensorRT PLAN, not an inductor cell: TRT serializes kernels for "
        "one device unless built in hardware-compatibility mode, so the SKU "
        "pin there is the real requirement and not a proxy for the arch.",
}

#: Modules on an adoption path. A ``sku`` refusal in any of them is the bug.
_ADOPTION_SOURCES = (
    "aot_serve.py", "aot_cells.py", "compile_cache.py", "fleet_cells.py",
    "local_cells.py", "trt_engine.py", "receipts.py", "hot_swap.py",
    "models/provision.py",
)


def _sku_comparison_sites(path: Path) -> List[Tuple[str, int]]:
    """(enclosing function, lineno) for every live-code site that COMPARES
    against the string ``"sku"`` — an equality/membership test, or a tuple
    of axis names iterated over. Prose and dict payloads are observability
    and stay: recording the SKU is required, refusing on it is the defect.
    """
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            for child in ast.walk(node):
                scope.setdefault(child, node.name)
    out: List[Tuple[str, int]] = []
    for node in ast.walk(tree):
        if isinstance(node, ast.Compare):
            probe: List[ast.AST] = [node]
        ...
        for sub in probe:
            for inner in ast.walk(sub):
                if isinstance(inner, ast.Constant) and inner.value == "sku":
                    out.append((scope.get(node, "<module>"), node.lineno))
                    break
    return out


def test_no_adoption_path_refuses_on_sku() -> None:
    unexplained: List[str] = []
    for rel in _ADOPTION_SOURCES:
        path = _SRC / rel
        for func, lineno in _sku_comparison_sites(path):
            if (Path(rel).name, func) in _ALLOWED_SKU_COMPARISONS:
                continue
            unexplained.append(f"{rel}:{lineno} in {func}()")
    assert not unexplained, (
        "these sites compare against 'sku' on an adoption path — a cell "
        "minted on another board with the same sm must ARM, not refuse "
        f"(pgw#765/pgw#691): {unexplained}")


# ---------------------------------------------------------------------------
# 2. verify — cross-SKU adopts, every real axis still refuses
# ---------------------------------------------------------------------------
