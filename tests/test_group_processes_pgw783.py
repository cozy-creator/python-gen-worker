...
from gen_worker.procsplit.parent import ParentControl  # noqa: E402
...
def test_parent_stays_torch_free_after_construction():
    """Constructing the parent imports no torch — it owns no GPU work.

    Measured in a SUBPROCESS: the suite's autouse ISA clamp (pgw#811 fallout)
    imports torch into every pytest process before the test body runs, so an
    in-process ``"torch" not in sys.modules`` can never observe the property
    again. A fresh interpreter is the only honest observer.
    """
    import os
    import subprocess
    import sys
    ...
    assert b"TORCH_FREE_OK" in out.stdout
...
def test_a_child_is_reaped_when_its_parent_dies():
    ...
    try:
        ...
        progress_wait.await_progress(
            _gone_yet,
            lambda reaped: reaped,
            what=f"kernel reap of grandchild {grandchild_pid} (PR_SET_PDEATHSIG)",
