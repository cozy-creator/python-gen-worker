"""An compiled graph-discovery MISS must start a mint, or refuse BY NAME.
...
1. **No producer.** If the serving path never reaches the supervised
   `mint_graph_classes` flow, a miss cannot produce the TCG artifacts the next
   pod needs and repeats forever.
2. **No declaration.** The mint refuses a family with no registered
...
from gen_worker import aot_mint, child_preflight, fleet_compiled_graphs, mint_process, mint_supervisor
...
def test_a_family_with_no_export_declaration_refuses_by_name(
    ...
    assert outcome.self_mint is None, "a JIT intake arm owes no compiled graph"
...
def test_every_execution_lane_a_pod_can_serve_reaches_the_aot_recipe(
    ...
    from AUTO until a compiled graph exists (th#1123 `applyCompileContractAvailability`,
...
def test_an_in_process_only_pod_declines_the_aot_recipe_by_name(
    ...
    assert outcome.self_mint is None, (
        "a pod that cannot delegate cannot mint an compiled graph — and pgw#1010 "
...
def test_the_parent_reemits_the_childs_aot_phase_table(
    ...
    assert {(k, p): ms for k, p, ms in rows}[
...
def test_the_child_runs_the_exporter_for_the_aot_recipe(
    ...
    # a `ck1` key names a 36-entry all-or-nothing compiled graph, which this
    ...
    def _fake_mint(template: Any, **kw: Any) -> Any:
        ...
        packed.write_bytes(b"tcg-artifact")
    ...
    # The worker declaration remains the source of class enumeration; register
    # a real public declaration rather than patching a deleted mint-local loader.
    register_export_declaration(_declaration())
    monkeypatch.setattr(
        fleet_compiled_graphs.loading, "pipeline_weight_lane", lambda pipe: "w8a8")

    from gen_worker.mint_process import MintRequest, cfg_spec
    ...
    assert Path(moved_path).read_bytes() == b"tcg-artifact"
...
def test_a_self_minted_aot_compiled_graph_arms_through_the_aot_gates(
    ...
    # a READABLE envelope. This used to be `b"compiled graph"` and worked only
