...
- **Model bindings** — `model` and `processor` are loaded from the module-level `Repo(...)` binding declared in `@inference(models={...})` and passed in as function arguments; the worker handles loading and caching.
...
- **GPU-bound inference** — the `endpoint.toml` build profile declares `accelerator = "cuda"` + `cuda = "12.8"`, and the `@inference`'s per-function `Resources` declares the VRAM floor.
