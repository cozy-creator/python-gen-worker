...
class MicroPipeline:
    ...
    def unpatchify(self, graphs: torch.Tensor, grid: int) -> torch.Tensor:
        ...
        batch = graphs.shape[0]
        out = graphs.reshape(batch, grid, grid, scale, scale, 3)
    ...
    def __call__(
        ...
        *,
        ...
        with torch.no_grad():
            graphs = self.decoder(x[-1][None, ...])
        return self.unpatchify(graphs, grid)
