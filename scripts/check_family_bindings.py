...
live in the two-minute required job rather than behind the fifteen-minute one.
    ...
    python scripts/check_family_bindings.py
...
from gen_worker.family.codegen import render_module  # noqa: E402
from gen_worker.family.snapshot import FamilyExport  # noqa: E402

CATALOG = REPO / "src" / "gen_worker" / "family" / "catalog"
...
def main() -> int:
    ...
    if unlisted:
        problems.append(
            f"{unlisted[0]}.export.json is committed but scripts/check_family_bindings.py "
    ...
    for family in sorted(found & set(SPECS)):
        ...
        if raw != export.dumps():
            problems.append(
                ...
                f"`gen-worker family export {SPECS[family]}`"
            )
        ...
        if actual != expected:
            problems.append(
                ...
                f"    gen-worker family generate "
                f"src/gen_worker/family/catalog/_generated/{family}.export.json "
                f"--spec {SPECS[family]}"
            )
