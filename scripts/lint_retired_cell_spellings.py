...
THE DEFERRED SET IS NOW SIX HISTORICAL NOTES, and that is the whole story of
...
What is left carries a proof only because it is a RECORD: six lines that name
the deleted env, the pre-migration on-disk path a stale pod volume may still
hold, or the exact names of tests pgw#1181 deleted. Respelling any of them would
falsify the record. So the marker's meaning has changed from "deferred, pending
a coordinated lane" to "historical, deliberately", and the count should stay at
six unless something is genuinely being recorded.
