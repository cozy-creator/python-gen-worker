...
the compiled-graph key grammar (:mod:`torch_compiled_graphs.identity`) both need the
...
#: (:func:`gen_worker.compile_cache.parse_cell_ref` routes through
...
#: need the hub to appear: ``parse_cell_ref`` raises on the fragment regex and
