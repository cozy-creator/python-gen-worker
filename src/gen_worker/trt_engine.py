...
``build`` is the producer for training-endpoints' PRICED ``produce-trt-engine``
function (pricing.yaml:555) — pgw#1179 deleted it as call-site-free, which was
true in-repo and false cross-repo (conversion/trt_engine.py:70). Restored
2026-08-12 (pgw#1180); whether the product function retires WHOLE — pricing
row, conversion function, and this module's serve half together — is a queued
T3 decision. ``find_artifact`` stays deleted; nothing anywhere calls it.
"""
