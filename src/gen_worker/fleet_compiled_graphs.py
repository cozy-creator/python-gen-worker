"""Fleet self-mint compile compiled graphs (gw#587).
...
Under self-mint the arming policy for a compile-declared function becomes:
  ...
  3. MISS on a family with NO export declaration: **JIT intake** — arm the
     ...
     intake with honest cold boots. A JIT compiled graph had no possible consumer
...
from . import aot_identity, aot_serve, artifact_meta, compiled_graph_key, env_seal
...
#: arm token must never pass ``compiled_graph_key.is_key`` / the hub's
#: ``compilecache.IsCompiledGraphKey``, because it is NOT a compiled graph key — see
#: :class:`ArmIdentity`.
#:
...
def declared_envelope_block(cfg: Any) -> Dict[str, Any]:
    ...
    ``compiled_graph_key.EXPORT_ENVELOPE_KEY`` agree by construction (canonical form:
    ``compiled_graph_key.envelope_facts``). GPU-gauntlet-proven: a fallback here that
...
# with the ARTIFACT, and at ~200 MB (a real AOT compiled graph) it crossed the 32 MiB
...
class CompiledGraphPublisher:
    ...
    def _post(self, path: str, payload: dict, *, timeout: float) -> dict:
        ...
        # `StateDelta.compiled_graph_lookups` — so these publish legs are the whole of
        ...
        with boot_mod.span(
            boot_mod.PHASE_COMPILED_GRAPH_HUB_RTT, function=path,
        ...
        if resp.status_code in (403, 429):
            # Typed refusals (compiled_graph_publish_forged_axis, _untrusted_tier,
    ...
    def publish(self, family: str, artifact: Path, meta: dict,
        ...
        # defense-in-depth): a compiled graph whose metadata carries a foreign
        # adoption provenance must never republish under this worker's
        # key. Nothing in-tree stamps the mark anymore (equivalence
        # adoption was deleted with exact identity); a marked compiled graph can only be a
        ...
        intent = self._post(
            "/v1/worker/compiled_graphs/publish-intent",
            {
                "family": family, "compiled_graph_key": key, "axes": axes,
                ...
                # `compiled_graph_key`. Before this, they reached the hub only on the
                # DEMAND side and were deleted the moment the demand was
                # satisfied — so a minted compiled graph's row could not say which lane
                ...
                # The compile wall for THIS compiled graph. Sent at INTENT, not
...
def _identity_axes(family: str, meta: dict) -> Dict[str, str]:
    """The identity map the hub records for this compiled graph, RECOMPUTED from the
...
def resume_owed_publishes(
    ...
    for compiled_graph in local_compiled_graph_store.compiled_graphs_owed_to_sink():
        ...
        if compiled_graph.key in in_flight:
            continue
        meta = artifact_meta.try_read_metadata(compiled_graph.artifact) or {}
        threads.append(_publish_async(
            publisher, compiled_graph.family or str(meta.get("family") or ""),
            compiled_graph.artifact, dict(meta), compiled_graph_key_digest=compiled_graph.key,
...
def delegation_refusal(pipe: Any, cfg: Any) -> str:
    ...
    aot-inductor compiled graph at all, and ``mint_recipe`` declines that by its own
    name.
    """
    try:
...
def _arm_candidate(
    ...
    *,
    ...
    pgw#923/#924. The `compiled_graph_arm` boot span and the adoption's `duration_ms`
    ...
    what an arm cost. `compiled_graph_arm` was a DECLARED boot phase with no producer at
    ...
    # inductor lanes, and bracketing those would put a near-zero `compiled_graph_arm` row
    # on every compile-declaring boot — the same "a default that reads as a
    # fact" defect pgw#924 is closing for `warmup`.
    span = (
        boot_mod.open_span(
            boot_mod.PHASE_COMPILED_GRAPH_ARM,
            ref=ref,
...
def arm_ordered(
    ...
    *,
    ...
    ``aot_compiled_graph`` arms exactly the pre-materialized ``artifact`` (its bytes
...
def _arming_policy(
    ...
    *,
    ...
    if recipe != RECIPE_AOT:
        ...
        # an export, because an AOT compiled graph is the only compiled graph there is.
...
def arm_axis_divergence(
    ...
    (pgw#1113). A compiled graph records no subject and must not: the key is the traced
    computation, so one compiled graph legally serves every checkpoint whose graph it
    is, and demanding the compiled graph restate the checkpoint it was minted from
...
def arm_from_local_store(
    ...
    * :data:`ROUTE_BOOT_KEY` — the ``ck1`` key §4.27's boot derivation produced
    ...
    route = ROUTE_MEMO
    try:
        local = local_compiled_graph_store.lookup_for_arm(arm_key.token)
...
def _admit_durable(
    ...
    activity_mod.emit_event(
        ...
        + ("" if compiled_graph.sink != local_compiled_graph_store.SINK_OWED
           else ", and the fleet upload is owed from these bytes"),
        phase=compiled_graph.sink,
    )
...
def withhold_self_mint_publish(pending: "PendingSelfMint", reason: str) -> None:
    ...
    hits=1/misses=1 -> compile_compiled_graph_failed -> release broken). The mint
...
def abandon_self_mint(pending: "PendingSelfMint") -> None:
    ...
    by scope, and is dropped only when a compiled graph actually ADOPTS. Keeping it past
...
_DELEGATION_DECLINE_DETAIL = {
    "no_eager_tier":
        "an armed non-eager backend (an AOTI compiled_graph) has replaced "
...
def mint_recipe(
    ...
    def _decline(reason: str, detail: str) -> str:
        ...
        if emit:
            activity_mod.emit_event(
                ...
                f"mint an aot-inductor compiled_graph — {detail}; this pod serves JIT "
                f"INTAKE instead (it compiles its own graphs and serves them "
                f"for its own life) and MINTS NOTHING — no compiled_graph, no key, no "
    ...
    # CYCLE: aot_mint imports CompiledGraphPublisher from this module at module scope,
...
def aot_export_spec(pipe: Any, cfg: Any) -> "Any":
    ...
    endpoint's own ``CompileCompiledGraph``. Everything the request file carried is
