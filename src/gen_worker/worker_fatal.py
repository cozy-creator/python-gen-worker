...
def report_worker_fatal(
    ...
    *,
    ...
    """Dial the hub with this process's cause of death."""
...
async def report_worker_error_async(settings: Optional[Settings], detail: str) -> bool:
    """Dial the hub from a STILL-LIVE worker that just entered WORKER_PHASE_ERROR — same carrier and durable ``pod_events`` row as the process-death fatal."""
    if settings is None or not (settings.orchestrator_public_addr or "").strip():
        return False
    if _broker_active():
        from .procsplit import broker
