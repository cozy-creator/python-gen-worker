...
class RequestContext(Generic[D]):
    ...
    def boot_warmup(self) -> bool:
        """True when this call is the worker's boot-time synthetic warmup : the output is discarded, so a handler MAY cheapen the run (e.g."""
        return self._boot_warmup

    @property
    def handles(self) -> Tuple[str, ...]:
        """The lane BODIES this function declared it branches on (``@entrypoint(handles=…)``), stamped from the spec at dispatch."""
        return self._handles

    @property
    def child_calls(self) -> bool:
        """Did this function declare ``@entrypoint(child_calls=True)``? The SDK mirror of the hub's ``invoke_child`` grant decision."""
        return self._child_calls

    @property
    def execution_lane(self) -> str:
        """The EXECUTING precision lane of this call, a full descriptor id like ``"fp8-w8a8-dynamic+compiled"`` — post-degrade truth, the same value JobMetrics.lane reports."""
    ...
    def config(self) -> Dict[str, Any]:
        """Effective values for this endpoint's declared config parameters (``@endpoint(config=[ConfigParam(...)])``) at dispatch time: declared defaults overlaid with the deployer-set values at the worker's ..."""
    ...
    def loras(self) -> Dict[str, Tuple[LoraOverlay, ...]]:
        """Per-request LoRA overlays riding each model slot: slot name -> tuple of ``{"ref", "weight"}``."""
        return _copy_context_metadata(self._loras)

    def adjusted(
        self, field: str, requested: Any, applied: Any, reason: str,
    ) -> None:
        """Record a caller-visible ADJUSTMENT: the serve path modified a requested value (clamp, substitution, injection)."""
    ...
    def adjustments(self) -> Tuple[Dict[str, str], ...]:
        """Immutable view of the adjustment ledger (:meth:`adjusted` / :meth:`clamp` rows, in emission order)."""
    ...
    def clamp(
        ...
        *,
        ...
        """Clamp ``requested`` into [lo, hi] and, when that CHANGES the value, record the adjustment (:meth:`adjusted`) — the one merge/ clamp helper endpoints use so caller-visible coverage cannot drift per ..."""
    ...
    def for_request(
        ...
        *,
        ...
        the instance scheduler's config, with ``sampler`` selecting from
        ``gen_worker.view.SAMPLERS``.

        THIS VIEW CARRIES NO OBJECTIVE: the body passes ``objective=""``
        unconditionally. An author who needs one passes it explicitly to the
        module-level gate ``gen_worker.view.for_request(..., objective=...)``,
        the only surface that honours it. EVERY sampler-shaped attribute is
        cloned, not just ``scheduler``; ``schedulers=`` pins the set when
        discovery is not wanted. Never assign ``self.pipeline.scheduler`` per
        request.
        """
    ...
    def device(self) -> "torch.device":
        """Torch device for this worker runtime (e.g."""
        try:
            import torch
        except Exception:
    ...
    def raise_if_cancelled(self, message: str = "request cancelled") -> None:
        """Raise ``CanceledError(message)`` if cancelled."""
        if self._canceled:
    ...
    def stage(self, name: str) -> Iterator[None]:
        """Bracket one stage of this request for :data:`JobMetrics.stage_ms` :: with ctx.stage("text_encode"): embeds = encode(prompt) The framework already times the permit wait, input fetch, denoise steps, ..."""
    ...
    def position(self, phase: str = "") -> Optional[float]:
        """The last position reported for ``phase``, or None."""
    ...
    def save_video(
        ...
        *,
        ...
        """Save an encoded video (bytes or a local file path); returns a typed :class:`VideoAsset` with probed container metadata (duration_s/fps/width/height/has_audio/sample_rate, best-effort)."""
...
class _PublisherMixin:
    ...
    def _download_blob_by_digest(
        ...
        """Fetch a blob by <algo>:<hex> digest to dest via the by-digest CAS read endpoint. The digest must be ALGORITHM-TAGGED: the hub keys two CAS namespaces on different algorithms (repo-CAS sha256, dataset-CAS blake3), so a bare hex string names no blob and is refused. Cap and verification are enforced DURING the stream: the response's declared Content-Length is the cap, checked per chunk, and a source with no Content-Length is refused (both hub arms declare one); `identity` encoding is demanded so declared and delivered lengths are the same quantity, making a gzip-bomb body impossible rather than merely detectable. Lands via a .part rename, so a refused fetch cannot leave a partial file a resume check would read as complete. `origin` is the provenance of the ADDRESS and alone decides how a terminal miss classifies."""
        import requests
    ...
    def checkpoint_dir(self, *, key: str) -> Path:
        """Return a JOB-SCOPED SCRATCH dir keyed by (job_id, key) — a stable working directory for trainer ``output_dir`` use within one job."""
    ...
    def resolve_dataset(
        ...
        """Materialize a dataset by bare dataset-id or ``owner/name`` ref; return the local root."""
...
class TrainingMetric(msgspec.Struct, frozen=True, kw_only=True):
    """Typed per-step training metric, payload of a ``request.training_metric`` event."""
...
class JobContext(_PublisherMixin, RequestContext[GenerationDefaults]):
    """The context a ``@job`` body receives — and a strict SUPERSET of what a producer-shaped ``@endpoint`` handler may use, under the same names."""
    ...
    def mktemp(self) -> Path:
        """Return a job-scoped scratch directory."""
