...
Each part is a member function of a generated ``_pgw811_run_ctx`` that
...
WAS = "//@pgw811:was:"
PARTS = "//@pgw811:parts"
CHUNK_BEGIN = "//@pgw811:chunk:begin:"
CHUNK_END = "//@pgw811:chunk:end:"
...
_DECLS: Sequence[Tuple[str, re.Pattern[str]]] = (
    ...
    ("const_bind", re.compile(
        r"^\[\[maybe_unused\]\]\s*auto&\s+(?P<n>\w+)\s*=\s*constants_->at\(\d+\);$")),
    ("kernels_bind", re.compile(
        r"^\[\[maybe_unused\]\]\s*auto&\s+(?P<n>\w+)\s*=\s*"
        r"static_cast<AOTInductorModelKernels&>\(\*this->kernels_\.get\(\)\);$")),
    ("array", re.compile(
        r"^(?:static\s+)?(?:constexpr\s+|const\s+)+int64_t\s+(?P<n>\w+)\[\]\s*=.*;$")),
    ("handle", re.compile(r"^AtenTensorHandle\s+(?P<n>\w+);$")),
    ("raii", re.compile(r"^RAIIAtenTensorHandle\s+(?P<n>\w+)\(\w+\);$")),
    ("steal", re.compile(
        r"^auto\s+(?P<n>\w+)\s*=\s*steal_from_raw_handles_to_raii_handles\(.*;$")),
    ("move", re.compile(
...
def find_anchors(lines: Sequence[str]) -> Anchors:
    inc = _find(lines, lambda s: s.startswith("#include <torch/csrc/inductor/aoti_include/"),
                what="aoti_include")
    drv = _find(lines, lambda s: s.startswith("#define CUDA_DRIVER_CHECK"), inc,
    ...
    kcls = _find(lines, lambda s: s.startswith("class AOTInductorModelKernels"), kopen,
                 what="AOTInductorModelKernels")
    kend = _find(lines, lambda s: s == "};", kcls, what="end of AOTInductorModelKernels")
    kclose = _find(lines, lambda s: s.startswith("}") and "namespace" in s, kend,
                   what="close of anonymous namespace")
    using = _find(lines, lambda s: s == "using namespace torch::aot_inductor;", kclose,
                  what="using namespace")
    ns2 = _find(lines, lambda s: s == "namespace torch::aot_inductor {", using,
                what="namespace #2")
    env = _find(lines, lambda s: s.startswith("static bool _check_aoti_runtime_check_inputs_env"),
                ns2, what="_check_aoti_runtime_check_inputs_env")
    env_end = _find(lines, lambda s: s == "}", env, what="end of runtime-check helper")
    run = _find(lines, lambda s: s.startswith("void AOTInductorModel::run_impl("), ns2,
                what="run_impl")
    run_end = _find(lines, lambda s: s.startswith("} // AOTInductorModel::run_impl"), run,
...
def parse_body(lines: Sequence[str], anchors: Anchors) -> Body:
    ...
    return Body(sig=list(lines[begin:body_begin]), prologue=(body_begin, prologue_end),
                compute=compute, decls=decls, uses=uses, params=params)


_INT_WORDS = frozenset({"L", "int64_t", "c10", "div_floor_integer", "static_cast"})


def resolve_type(lines: Sequence[str], body: Body, name: str, _seen: Optional[Set[str]] = None) -> str:
...
def cut_points(lines: Sequence[str], body: Body, parts: int) -> List[int]:
    ...
    for k in range(1, parts):
        ...
        while b < end and (b in open_groups or not lines[b].strip()
...
def _main_tu(lines: Sequence[str], a: Anchors, body: Body, plan: Plan) -> str:
    ...
    ctx_ctor = (f"    {CTX} {PREFIX}ctx{{this->constants_, this->kernels_, "
...
def reconstruct(main: str, parts: Sequence[str]) -> str:
    ...
    for line in rederived:
        if line not in original:
            raise Declined(
                f"re-derived line copies nothing in the original: {line.strip()[:70]}")
    return "\n".join(out)
