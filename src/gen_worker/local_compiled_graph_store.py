...
pre-migration tree is simply not found, and re-pays one mint.
...
#:
#: This store's subtree of that root. Distinct from ``compiled-graphs``, which
#: ``worker.py`` and the CLI already use for the v2 artifact pool — two stores
#: under one root must not share a directory name.
