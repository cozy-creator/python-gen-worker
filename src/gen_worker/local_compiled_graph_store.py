"""pgw#1096 / §4.28 — the untrusted machine's OWN compiled graph store: AOT compiled graphs, ck1-keyed.
...
addressing, so a compiled graph that later becomes publishable needs no re-keying and a
...
(``local_compiled_graphs``). A compile compiled graph is user-generated EXECUTABLE code
...
books it as SUNK — *"a sealed compiled graph was produced and thrown away"* — and
``fleet_compiled_graphs._publish_async``'s ``finally`` then rmtree'd the bytes.
...
unit and a partial write leaves nothing admissible)::
    ...
    <root>/aot-compiled graphs/<ck1-…>/compiled graph.tar.gz   the packed artifact
    <root>/aot-compiled graphs/<ck1-…>/record.json   {compiled_graph_key, content_digest, family, …}
...
child-minted compiled graph passes, so a wrong memo can only cost one re-mint. When
...
compiled graph per (graph x envelope x sm x toolchain), so every torch upgrade, every
gen-worker upgrade and every settings change leaves the previous compiled graph resident
forever. :func:`stored_compiled_graphs` enumerates them with sizes so a future sweep has
a fact base; an age-based sweep would be wrong (a compiled graph nobody booted for six
months is still exactly the compiled graph the next boot wants).
"""
...
#: compiled graph is the only kind with an identity, so it is the only kind that can be
...
def store_root() -> Path:
    ...
    Moved here from ``local_compiled_graphs`` by pgw#1096 with its value UNCHANGED —
    same env name, same default — so ``aot_resume``'s crash-only bank and
    cozy-local's ``cozy compiled graphs`` CLI keep resolving to the identical
    directory. That move is what makes ``local_compiled_graphs.py`` deletable by
...
def compiled_graph_dir(key: str, root: Optional[Path] = None) -> Path:
    """The directory holding the compiled graph keyed ``key``.
...
class LocalCompiledGraph:
    """One compiled graph resident in the local store, with the facts recorded beside it."""
...
def store(
    ...
    Returns the stored compiled graph, or ``None`` when the store failed: a local store
...
def _compiled_graph_of(key: str, record: Dict[str, Any], artifact: Path,
             digest: str) -> LocalCompiledGraph:
...
def mark(
    ...
    """Move a stored compiled graph's ``verdict`` and/or ``publish`` state (pgw#1183).
    ...
    conservative one in both directions (an unpromoted compiled graph is re-minted; an
...
def lookup(key: str, root: Optional[Path] = None) -> Optional[LocalCompiledGraph]:
    """The resident, ADMITTED compiled graph for ``key``, or ``None``.
    ...
    is how an unverified compiled graph heals.
    """
    try:
        target_dir = compiled_graph_dir(key, root)
    except ValueError:
    ...
    if not want or have != want:
        ...
        drop(key, root)
        return None
    return _compiled_graph_of(key, record, artifact, have)
...
def note_memo(
    ...
    of a mint: the sweep deletes the shortcut and leaves the COMPILED GRAPHS under their
    own keys, so the next boot's derived key re-finds the compiled graph and re-writes
...
def drop(key: str, root: Optional[Path] = None) -> None:
    """Remove one compiled graph and everything recorded about it."""
    try:
        shutil.rmtree(compiled_graph_dir(key, root), ignore_errors=True)
    except ValueError:
        return


def stored_compiled_graphs(root: Optional[Path] = None) -> List[LocalCompiledGraph]:
    """Every resident compiled graph, cheaply — NO digest recomputation.
    ...
    listing a ``cozy compiled graphs``-style CLI and any future sweep read, and making it
    hash every resident artifact would make listing cost as much as arming.
    """
    out: List[LocalCompiledGraph] = []
...
def quarantined_compiled_graphs(root: Optional[Path] = None) -> List[LocalCompiledGraph]:
    """Every compiled graph kept for FORENSICS: it existed, and it failed its own gate.
...
def compiled_graphs_owed_to_sink(root: Optional[Path] = None) -> List[LocalCompiledGraph]:
