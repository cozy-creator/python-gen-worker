"""Native-kernel implementation and conservative dispatch.

TCG format 1 has one closed artifact schema and deliberately carries no
worker-local kernel-choice block. The removed worker artifact format tried to
smuggle a measured ``kernel_lane`` beside TCG metadata; TCG rejected that
extension, so every real artifact silently selected the baseline anyway.

Make that effective policy explicit: both production decisions are baseline.
The fused/packed implementations and their direct benchmark seams remain, but
they are not armed by a second artifact metadata authority.
"""
...
MOD_DENSE = "dense"
_BASELINE_REASON = (
    "compiled-graph format 1 carries no worker-local kernel-lane policy; "
    "serving the deterministic baseline"
)
...
def _decide(
    ...
    """Return the closed-format baseline, cached with its exact reason."""
    ...
    logger.info("native kernels [%s]: %s (%s)", axis, off, _BASELINE_REASON)
    return _record(axis, off, _BASELINE_REASON)


def svdq_linear_execution_lane() -> str:
    """``"fused"`` | ``"baseline"`` for the W4A4 linears in this process.
    Format 1 always selects the deterministic baseline."""
    return _decide(AXIS_LINEAR, LINEAR_BASELINE)


def svdq_modulation_execution_lane() -> str:
    """``"packed"`` | ``"dense"`` for W4A16 AdaLN modulation.
    Format 1 always selects dense modulation."""
    return _decide(AXIS_MODULATION, MOD_DENSE)


def svdq_linear_execution_lane_reason() -> str:
    """The recorded reason for the linear-lane decision."""
    svdq_linear_execution_lane()
    return _REASONS[AXIS_LINEAR]
...
def svdq_modulation_execution_lane_reason() -> str:
    ...
    return _REASONS[AXIS_MODULATION]
