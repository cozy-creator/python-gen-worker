...
guard keeps LoRA-bearing pipelines and branchless compile compiled graphs apart in
both directions.
...
def effective_base_execution_lane(pipe: Any) -> str:
    ...
    pgw#686: the advertised requested compiled graph key and the minted/published compiled graph
    key MUST resolve the base lane identically — when they don't, the
    published compiled graph is never requested by any worker, adoption is
