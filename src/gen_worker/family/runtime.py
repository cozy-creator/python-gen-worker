"""``FamilyInstance`` — the value a handler parameter actually receives.
...
from .snapshot import ExportedVariant, FamilyExport
...
class InstanceResolver(Protocol):
    ...
    an author's laptop) has none — which is exactly when :meth:`FamilyBinding.
    fake` is the right answer rather than a silently-degraded real one.
    """

    def __call__(self, family: Family | None, name: str, ref: str) -> Backing: ...
...
class DecodeSession:
    ...
    def __init__(self, instance: FamilyBinding) -> None:
    ...
    @property
    def instance(self) -> FamilyBinding:
        self._require_open()
...
class FamilyBinding:
    """Base of every GENERATED family class: the type and the instance in one.
    ...
    * nothing mutable. Class-level output is immutable identity, full stop.
    ...
    There is **no public constructor** (torchcg G12): a family cannot be called
    ...
    SPEC: ClassVar[Family | None] = None
    ...
    def __init__(self) -> None:
        raise FamilyError(
            FamilyRefusal.BACKING_MISSING,
            f"{type(self).__name__} is a family TYPE; a value of it is a resolved instance. "
...
def resolve(payload: object, instance: FamilyBinding) -> dict[str, Any]:
