...
:func:`fake_families` is the same shape with the fake backing, and it is what
...
def declared_families(target: object) -> dict[str, type[FamilyBinding]]:
...
def bind_families(
    ...
    resolved: Mapping[str, FamilyBinding],
) -> dict[str, FamilyBinding]:
...
def fake_families(
    families: Mapping[str, type[FamilyBinding]], *, seed: int = 0
) -> dict[str, FamilyBinding]:
...
def fake_kwargs(target: object, *, seed: int = 0) -> dict[str, FamilyBinding]:
    """:func:`fake_families` for a decorated endpoint, by declaration."""

    return fake_families(declared_families(target), seed=seed)
...
def resolver_instances(
    families: Mapping[str, type[FamilyBinding]], refs: Mapping[str, str]
) -> dict[str, Any]:
