...
from .snapshot import FamilyExport


def assert_recipe(export: FamilyExport, recipe: Recipe) -> None:
    """Refuse a mint-emitted recipe that drifted from the binding source.

    Raises :class:`FamilyError` with :attr:`FamilyRefusal.DECLARATION_DRIFT` on
    ...
    if str(recipe.family) != str(export.family):
        raise FamilyError(
            FamilyRefusal.DECLARATION_DRIFT,
    ...
    try:
        ...
        raise FamilyError(FamilyRefusal.DECLARATION_DRIFT, str(exc)) from exc
