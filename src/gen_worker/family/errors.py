"""The closed refusal set for the typed Family SDK.

Every refusal this package can produce is a member of :class:`FamilyRefusal`.
...
class FamilyRefusal(StrEnum):
    """Every reason the Family SDK refuses a declaration, export or binding."""
...
class FamilyError(ValueError):
    ...
    Carries the closed :class:`FamilyRefusal` reason as ``.reason`` so callers
    branch on the reason rather than on message text — the same shape
    ``torchcg.RecipeError`` uses, for the same reason.
    """

    def __init__(self, reason: FamilyRefusal, detail: str) -> None:
        self.reason = reason
        super().__init__(f"{reason.value}: {detail}")


__all__ = ["FamilyError", "FamilyRefusal"]
