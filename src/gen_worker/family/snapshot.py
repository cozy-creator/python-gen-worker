...
def _canonical(document: Mapping[str, Any]) -> bytes:
    ...
    try:
        ...
        raise FamilyError(
            FamilyRefusal.SNAPSHOT_INVALID, f"export document is not finite JSON: {exc}"
        ) from exc
...
def _fields(
    ...
    if not isinstance(raw, Mapping):
        raise FamilyError(FamilyRefusal.SNAPSHOT_INVALID, f"{kind} must be an object")
...
def _parse(kind: str, value: object, parse: Any) -> str:
    try:
        ...
        raise FamilyError(
            FamilyRefusal.SNAPSHOT_INVALID, f"{kind} {value!r} is not canonical: {exc}"
        ) from exc
...
class ExportedOutput:
    ...
    def __post_init__(self) -> None:
        if not self.dtype or self.dtype != self.dtype.strip():
            raise FamilyError(FamilyRefusal.SNAPSHOT_INVALID, "output dtype must be canonical")
        ...
        if any(
            ...
            raise FamilyError(FamilyRefusal.SNAPSHOT_INVALID, "output shape is not canonical")
    ...
    def decode(cls, raw: object) -> ExportedOutput:
        ...
        if not isinstance(shape, list):
            raise FamilyError(FamilyRefusal.SNAPSHOT_INVALID, "output shape must be an array")
...
class ExportedVariant:
    ...
    def __post_init__(self) -> None:
        ...
        if names != tuple(sorted(set(names))):
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_INVALID, "bucket axes must be sorted and unique"
            )
    ...
    def decode(cls, raw: object) -> ExportedVariant:
        ...
        if not isinstance(bucket, Mapping) or not isinstance(outputs, list):
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_INVALID, "variant bucket/outputs are malformed"
            )
        try:
            ingress = CallIngress.decode(row["ingress"])
        except IngressError as exc:
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_INVALID, f"variant call ingress is invalid: {exc}"
            ) from exc
        return cls(
            bucket=tuple(
                sorted(
...
class ExportedRunner:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.variants:
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_INVALID, f"runner {self.name!r} exported no variant"
            )
        ...
        if len(shapes) != 1:
            ...
            raise FamilyError(
                FamilyRefusal.SIGNATURE_DISAGREEMENT,
    ...
    def decode(cls, raw: object) -> ExportedRunner:
        ...
        if not isinstance(axes, list) or not isinstance(variants, list):
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_INVALID, "runner axes and variants must be arrays"
            )
        return cls(
...
class ExportedStage:
    ...
    def decode(cls, raw: object) -> ExportedStage:
        ...
        if repeat not in tuple(kind.value for kind in RepeatKind):
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_INVALID, f"unknown loop repeat {repeat!r}"
            )
...
class ExportedLoop:
    ...
    def decode(cls, raw: object) -> ExportedLoop:
        ...
        if kind not in tuple(item.value for item in LoopKind):
            raise FamilyError(FamilyRefusal.SNAPSHOT_INVALID, f"unknown loop kind {kind!r}")
        if session_state not in tuple(item.value for item in SessionState):
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_INVALID, f"unknown session state {session_state!r}"
            )
        if not isinstance(stages, list) or not stages:
            raise FamilyError(FamilyRefusal.SNAPSHOT_INVALID, "loop stages must be an array")
        return cls(
            kind=LoopKind(kind),
...
class ExportedScheduler:
    ...
    def decode(cls, raw: object) -> ExportedScheduler:
        ...
        if not isinstance(parameters, Mapping):
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_INVALID, "scheduler parameters must be an object"
            )
        return cls(
...
class FamilyExport:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.runners:
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_INVALID, "an export must declare at least one runner"
            )
        runner_names = tuple(runner.name for runner in self.runners)
        if runner_names != tuple(sorted(set(runner_names))):
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_INVALID, "runners must be sorted by name and unique"
            )
        ...
        for runner in self.runners:
            ...
            for layout in runner.layouts:
                ...
                if missing:
                    raise FamilyError(
                        FamilyRefusal.BUCKET_COVERAGE_INCOMPLETE,
    ...
    def runner(self, name: str) -> ExportedRunner:
        ...
        for runner in self.runners:
            if str(runner.name) == wanted:
                return runner
        raise FamilyError(
            FamilyRefusal.SNAPSHOT_INVALID, f"export declares no runner {wanted!r}"
        )
    ...
    @classmethod
    def decode(cls, raw: object) -> FamilyExport:
        row = _fields(
            "family export",
            raw,
        ...
        if type(version) is not int or version != EXPORT_VERSION:
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_VERSION_UNSUPPORTED,
        ...
        for name in ("buckets", "runners", "parameters"):
            if not isinstance(row[name], list):
                raise FamilyError(
                    FamilyRefusal.SNAPSHOT_INVALID, f"family export {name} must be an array"
                )
        ...
        for axis in row["buckets"]:
            ...
            if not isinstance(values, list) or not values:
                raise FamilyError(
                    FamilyRefusal.SNAPSHOT_INVALID, "bucket values must be a non-empty array"
                )
            buckets.append(
                (
    ...
    @classmethod
    def loads(cls, payload: bytes | str) -> FamilyExport:
        try:
            document = json.loads(payload)
        except ValueError as exc:
            raise FamilyError(
                FamilyRefusal.SNAPSHOT_INVALID, f"family export is not JSON: {exc}"
            ) from exc
