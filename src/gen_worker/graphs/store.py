...
from .._vendor.tensorfs import CASRef, DigestMismatch, LocalCAS, RefConflict

from .document import DocumentError, GraphSetDocument, LaneGraphs
from .._vendor.torchcg.identity import is_graph_hash
from .._vendor.torchcg.refuse import StoreError
from .env import ArtifactEnv
from .lane import LaneError, require_pass_ref
from .requirements import RequirementsError, RequirementsManifest

logger = logging.getLogger(__name__)

#: This store's ref namespace. Still spelled `torchcg/` after the tcg#90 move
#: because it names REFS THAT EXIST on disk, and renaming it would orphan every
#: banked program and document for nothing. It does not collide with the
#: artifact store torchcg keeps under `torchcg/v1/graphs`.
_REF_PREFIX = "torchcg/v2"
#: One I/O buffer for the whole-blob reads below. torchcg owns the same
#: constant for its own envelope reads; this is a different reader over
#: different files, so it states its own rather than importing one to share a
#: literal (tcg#86: a shared literal is an invitation to unify two policies).
_READ_BUFFER = 1 << 16
...
class GraphStore(Protocol):
    ...
    def fetch_artifact(self, graph: str, env: ArtifactEnv, destination: str | Path) -> Path | None:
    ...
    def get_manifest(self, graph: str, env: ArtifactEnv) -> RequirementsManifest | None:
        """Return the manifest published beside (graph, env), or a miss."""
        ...


def holes(store: GraphStore, lane: LaneGraphs, env: ArtifactEnv) -> tuple[str, ...]:
...
def _artifact_ref(graph: str, env: ArtifactEnv) -> str:
    return f"{_REF_PREFIX}/artifacts/{_require_graph(graph)}/{env.value}"


def _manifest_ref(graph: str, env: ArtifactEnv) -> str:
...
class LocalGraphStore:
    ...
    def artifact_skew(self, graph: str, env: ArtifactEnv) -> str | None:
    ...
    def _ensure_manifest(self, graph: str, env: ArtifactEnv, candidate: CASRef) -> None:
