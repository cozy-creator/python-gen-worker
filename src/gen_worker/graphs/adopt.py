...
from .._vendor.torchcg.adopt import load as _tcg_load
from .._vendor.torchcg.refuse import AdoptError as ArtifactError
from .._vendor.torchcg.store import open_artifact
...
#: Binding a constant table to the live module is torchcg's job now, and it
#: raises one type for every way that fails.
ConstantBindingError = ArtifactError
...
class _ForwardDispatcher:
    ...
    __slots__ = ("eager_forward", "module", "_entries", "_tcg")
    ...
    def __init__(self, module: Any) -> None:
        ...
        #: The ONE dispatch path (se#837). This used to be a second
        #: implementation of admission living here, and it refused any dtype
        #: disagreement outright — so an int64 timestep from ddim/dpmpp/unipc
        #: served eager while the contract said it was covered. torchcg's
        #: dispatcher decides admission and applies the normalization with the
        #: same pass, so a computable-but-unexecuted plan cannot exist. It also
        #: owns the once-per-module first-miss diagnosis (tcg#76).
        self._tcg = _TCGDispatcher(eager=module.forward)
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any:
        return self._tcg(*args, **kwargs)

    @property
    def compiled_calls(self) -> int:
        return self._tcg.compiled_calls

    @property
    def eager_calls(self) -> int:
        return self._tcg.eager_calls

    @property
    def recast_calls(self) -> int:
        """Calls served COMPILED after a boundary normalization (se#837)."""

        return self._tcg.recast_calls
...
class AdoptSession:
    ...
    def __init__(
        ...
        *,
        ...
        self.env = require_env(document.stack, sm)
        ...
        self.passes = self._reconcile_passes(lane)
        self._store = store
        self._loader = loader
    ...
    @staticmethod
    def _reconcile_passes(lane: LaneGraphs) -> tuple[str, ...]:
        """A lane's declared passes, reconciled against what ran.

        Nothing in this repo runs a transform pass, so the sealed-set half of
        this went with torchcg's `transform.py` (tcg#90). What survives is the
        refusal that still fires: a lane DECLARING passes when none ran is
        addressing a graph nobody can reproduce, because a pass name is a
        derivation input of the graph hash.
        """

        from .lane import PassOrderError, require_declared_passes

        try:
            return require_declared_passes(lane.contract, lane.passes)
        except PassOrderError as exc:
    ...
    def _load(self, dispatcher: _ForwardDispatcher, artifact: Path, record: GraphRecord) -> Any:
        ...
        loader = self._loader
        if loader is None:
            def loader(path: Path, _record: GraphRecord, module: Any) -> Any:
                # The envelope came through pgw's own transport (hub, baked
                # layer, staged volume), so it is OPENED rather than fetched
                # from torchcg's store — same unpack, same refusals — and then
                # bound against THIS module's live weights (user_managed).
                staged = Path(path).with_suffix(".unpacked")
                return _tcg_load(open_artifact(Path(path), staged), module)
