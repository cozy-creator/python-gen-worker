"""Repo-root pytest plugin: the skip census (pgw#963).
...
def pytest_addoption(parser: pytest.Parser) -> None:
    parser.addoption(
        ...
        default=None,
        metavar="PATH",
        help="pgw#963: write this session's skip census (JSON) to PATH",
    )
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.pluginmanager.register(SkipCensus(Path(str(out))), "pgw963-skip-census")


def pytest_sessionfinish(session: pytest.Session) -> None:
    plugin = session.config.pluginmanager.get_plugin("pgw963-skip-census")
