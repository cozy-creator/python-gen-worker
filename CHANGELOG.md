...
## 0.112.0 (2026-08-11) — **an adopted cell is read off the OBJECT, so a boot-adopt actually serves compiled** — plus the author's declared speed bar and open mint blockers on the wire
...
## 0.111.0 (2026-08-11) — **a boot-adopted cell now SERVES** — the warmup proof barrier is deleted, the parity gate moves to the pod that compiled the bytes, and an operator can turn compiled serving off on a live worker
...
## 0.108.0 (2026-08-11) — **the execution axis is REPORTED, not derived** — plus the rig/probe pin folds. Tagged `v0.108.0` at `821090c3` and published to PyPI 2026-08-11 18:09Z; it does NOT carry pgw#1133 (see 0.109.0)
...
## 0.106.0 (2026-08-11) — **a split serving pod knows who it is, a boot-trace child stops fighting for the card, and the boot key stops depending on a package nothing declared**
...
## 0.105.0 (2026-08-11) — **the clone producer can size and publish a mirror repair again** — an untagged fp32 upstream is no longer read as 4-bit, and a pod's own warm pass no longer arrives as the caller's fatal
...
## 0.104.0 (2026-08-11) — **the obligation names its SUBJECT, and a mint REFUSAL is DATA** — cell/arm identity soundness with a `arm1`→`arm2` memo bump (pgw#1113), declarative `blockers=` on `Compile` (pgw#1115), typed boot-adopt outcomes plus the `declaration_refused` serving fix (pgw#1116), and a fail-closed weight-free export (pgw#1080)

- **pgw#1080 increment 3: a weight-free-buildable target that is NOT honored
  fails closed, instead of degrading to a real-weight export.** The pod-proof
  lane measured z-image's self-mint exporting `ZImageTransformer2DModel` with
  ~40 GiB of REAL device memory resident, misclassified `retryable` (ie#638) —
  even though `build_component` + `init_empty_weights` + `from_config` builds
  that class fully weight-free (proven cardless, 0 real param bytes). Two holes,
  one cause: the meta gate wrapped the export only when a fake mode was already
  present, so a real-weight export ran UNGUARDED and its OOM read as retryable;
  and `mint_child._load` swallowed `_assert_structure_honored`'s refusal into
  the same fallback as a genuinely stranded family. The refusal is now typed —
  `StructureNotHonored` (a `StructureOnlyUnsupported` subclass) — and
  `mint_child._load` catches it FIRST and raises `MintChildRefused` naming the
  authoring/composition cause, so the export never starts. A genuinely stranded
  family (no config surface, w8a8/w4a4/svdq/gguf artifact lane, unnamed class)
  still raises the plain parent and still falls back, matching
  `boot_trace_child`, which already failed closed on this case
  (`tests/test_structure_only_pgw1080.py`).
...
## 0.103.0 (2026-08-11) — **closes the AOT reuse circle** — the boot-adopt credential gate (pgw#1108), the declaration-inference SDK (pgw#1107), `metrics.lane` follows the weights AS EXECUTED (pgw#1104), and the traced graph body folds into the cell key (pgw#1031)
...
## 0.102.0 (2026-08-10) — **the output-integrity floor is a SERVE-path floor** (th#1771) — a warm forward's discarded output is not a render that can bank, and the derived warm payload must satisfy the payload's OWN invariants
...
## 0.101.0 (2026-08-10) — **`PromptRole` is exported at top level** (th#1757) — a MINOR because the fleet's `[supported]` allowlist is keyed on MAJOR.MINOR

- **th#1757: `PromptRole` joins `gen_worker.__all__`.** The reference layer's
  role enum was importable only from its defining module, so an endpoint
  declaring `accepts_references=` had to reach past the public surface. Adding a
  name to `__all__` is why this is a minor and not a patch: endpoint images
  select the SDK through a `[supported]` allowlist keyed on MAJOR.MINOR, so a
  new public name needs a version the allowlist can name.
- The th#1771 warm-payload fix was tagged here as well; it is documented under
  0.102.0, which is the release that completes it.

## 0.100.1 (2026-08-10) — **an unreadable cell envelope is a refusal, not an absence** (pgw#1098, the row-7 drain fix: `MAX_METADATA_BYTES` 16→64 MiB) — plus the fail-open sweep (pgw#939, pgw#887, pgw#894, pgw#498)
...
## 0.100.0 (2026-08-10) — **the AOT re-key** — the boot-derived cell key (pgw#1089), the folding fence (pgw#1097), AOT-local mint (pgw#1096) and the weightless compile-target export (pgw#1080)
...
## 0.99.0 (2026-08-10) — **minted-but-never-installed, and the degrade that reached the wire as nothing** (pgw#1093) — plus the twelve-phase cold-boot decomposition (pgw#1087) and the typed fixed-slot resolution failure (pgw#1088)
...
## 0.98.0 (2026-08-10) — **the compiled lane that served 100% eager and reported `jit_cell`** (pgw#1082) — declared marks that raise on every call, a regional guard that confesses its degrade, and every resident slot staged around the mint child (pgw#1079)
...
## 0.97.0 (2026-08-10) — **armed at setup, eager at serve** — the regional lane that declined itself and the scope-armed slot that owned nothing (pgw#1078), on top of the settings-authority seal (pgw#1049), the unbounded-read sweep (pgw#973/pgw#1013) and the mint-lane train
...
## 0.96.3 (2026-08-10) — **a REFUTED equality is not a pin** — the declared-range gate reads the axiom's VALUE (pgw#1077), unblocking z-image's mint on the ie#637-fixed endpoint
...
## 0.96.2 (2026-08-09) — **a scalar timestep's dtype is a SAMPLER fact, normalized at the ingress — and a refusal names the CLOSEST entry** (pgw#1074, the pgw#868 A1 close)
...
## 0.96.1 (2026-08-09) — **a cell that admits none of its own labelled classes can no longer be published, and the silent dtype guess that minted one is unconstructable** (pgw#1058, the last pgw#868 A1 gate)
...
## 0.96.0 (2026-08-09) — **stamped cell identity stops fracturing per host, and a 36-entry AOT cell arms without N copies of its constants** (pgw#1042, the pgw#868 A1 attempt-28 fix)
...
## 0.95.1 (2026-08-09) — **a model load can no longer run silent, and a SIGKILL'd load names where it died** (pgw#1041, the ie#615 attempt-4 fix) — plus the Plan / artifact-verify / mint-lane cleanup train (pgw#902, pgw#903, pgw#1032-1035, pgw#1040)
