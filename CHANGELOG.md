...
## 0.93.2 (2026-08-06) — **adoption stops being a pilot** (pgw#990/pgw#988): the flag that gated it deleted, the declare/verify contract split, and `code_closure` out of the key

> Branched from `v0.93.1`, NOT from `master`: master carries th#1597's
> `cozy.scheduler.v1` wire cut, and a worker built from it cannot talk to the
> current pre-v1 dev hub. This patch is deliberately pre-v1 so the pgw#868 A1
> pod run can use it. The same changes land on `master` by PR in parallel.

- **pgw#990: AOT cell adoption was behind a default-OFF pilot flag, and the
  flag stopped arriving on 2026-08-05 — so the fleet minted the WRONG KIND of
  cell and never looked for one.** `aot_cells.prefer_aot()`
  (`Settings.compile_prefer_aot` / `GEN_WORKER_PREFER_AOT`) stood in front of
  two call sites, not one: `fleet_cells.enable_compiled`'s discovery branch,
  **and `fleet_cells.mint_recipe`'s first line**, which returned
  `RECIPE_DYNAMO` whenever it was off. The flag reached pods through a
  release-scoped `endpoint_env` entry that had to both match the release
  (`applies_to`) and be declared by it — and neither held for any release
  after `bdf54080…`. Measured consequence on the dev fleet: the last
  `aot_cell_discovery` event of any kind is 2026-08-02, three A1 attempts ran
  with adoption un-armed, and the one cell that got published carries
  `kind="torch-inductor-cache"` — a dynamo envelope that AOT discovery rejects
  by name. `pgw#805`'s docstring had predicted the loop verbatim: *"a fleet
  missed, re-minted the wrong kind (or nothing), and missed identically on
  every subsequent pod, forever."* **The gate is DELETED**, not re-pointed:
  the Settings field, the env name, the loader mapping, the predicate and both
  call sites. `provision`'s lifted-LoRA install now keys off the artifact's own
  bucket (it was `bucket and compile_prefer_aot`, so an adopted bucket-bearing
  cell reached `aot_serve.enable` UNLIFTED on exactly the pods the flag had not
  reached). A pod still handed the retired env name now reports it as
  unrecognised owned env, so a stale row is loud instead of silent.
- **pgw#988: the publish declare and the discovery filter were one contract
  with no single authority, and they disagreed — so no cell published from
  `c2e52f5f` on was adoptable by anything.** th#1645 correctly dropped the
  unbounded `entries` map from the control-plane declare; `aot_cells` went on
  verifying the FULL contract against that declare and rejected every cell with
  `malformed declared contract: metadata declares no entries map`. The contract
  is now two named levels. `aot_serve.verify_declared` is the PREFILTER: it
  rules on exactly what a bounded declare can state (format, kind, weights-baked
  refusal, identity axes, host ISA, family) plus the new bounded
  `entries_digest` + `entries_count`, which `control_plane_metadata` folds in at
  the same seam that strips the map — so the two halves are written in one place
  and cannot drift apart again. `aot_serve.verify` stays the GATE and now
  actually runs where the entries live: over the staged artifact's own metadata
  in `aot_serve.arm`, raising `contract_invalid`. Previously the full verify ran
  ONLY against the declare and never against the artifact. The prefilter fails
  open on a silent summary and the gate fails closed on every entry — a
  prefilter that refuses whatever it does not recognise is precisely what
  pgw#988 was.
- **pgw#990: `code_closure` leaves the cell key; `ck5` → `ck6`.** Paul's final
  cell-identity ruling is that identity is the COMPUTATION — traced graph × sm ×
  toolchain × env_seal — and that code hashes are a MEMO, never identity. The
  ck5 `code_closure` axis was a 147-file source-content hash, so every wheel
  release re-keyed every cell in the fleet for edits that cannot change a traced
  graph (0.93.0 → 0.93.1 moved the sdxl key on three plumbing files). It is
  dropped from the required axes, still RECORDED on every artifact, and still
  drives the local re-trace memo. `KEY_SCHEME` is bumped so older cells strand
  by name, once, instead of silently re-keying under a reused prefix.
  `cell_key.is_key` is now scheme-AGNOSTIC, byte-identical to tensorhub's
  `compilecache.IsCellKey` and for the reason th#1183 gives: pinning the current
  scheme turned every other-scheme cell into `unreadable_cell_key`, which is
  both a lie and a filter no axis justifies.
- **th#1647 is disproven and re-aimed.** No forge/serving lane divergence
  exists: a serving pod's own `aot_cell_discovery` event asks for
  `lane=w8a8-lora64`, the lane the forge mints. The `cell_demand` rows that
  suggested otherwise were four different keys wearing one hub-synthesized
  label (`candidateDemandAxes` builds one axes map per release+family and
  stamps it on every candidate key), in a different key space, for a different
  cell kind.
